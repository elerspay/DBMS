...
#define OPERATOR_UNARY 0x80
	...
	typedef struct group_by_item_t {
		char* column_name;           // 分组的列名
		struct group_by_item_t* next; // 下一个分组项
	} group_by_item_t;
	...
	typedef struct select_info_t {
		...
		expr_node_t* where;
		order_by_item_t* order_by;  // ORDER BY 子句
		group_by_item_t* group_by;  // GROUP BY 子句 (新添加)
	} select_info_t;
